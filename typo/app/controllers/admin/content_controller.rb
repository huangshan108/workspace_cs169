...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    unless (current_user.admin? && request.post?)
      redirect_to :action => 'index'
      flash[:error] = _("Sorry, you do not have the privilege to preform this action.")
      return
    end
    ...
    if has_merged
      ...
      flash[:error] = _("Sorry, the article your trying to merge is not found or is the current article.")
    end
    redirect_to :action => 'index'
  end
