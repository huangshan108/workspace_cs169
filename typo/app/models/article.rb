...
class Article < Content
  ...
  def merge_with(another_article_id)
    another_article = Article.find_by_id another_article_id
    if another_article && (another_article_id != self.id)
      Article.update(self.id, {:body => self.body + "<hr>" + another_article.body })
      another_article.comments.each do |comment| 
        self.comments << comment
      end
      Article.delete(another_article_id)
      return true
    end
    return false
  end
